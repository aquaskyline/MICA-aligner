...
inline int PESRAIsPairEndMatch(PEInput * peInput, SRAOccurrence * occ_1, SRAOccurrence * occ_2, unsigned int * insertion);
...
inline void PEMappingCore(PEInput * peInput, PEOutput * peOutput, PEStats * peStats, 
    ...
    for (occIndex_1=0;occIndex_1<occCount_1;occIndex_1++) {
        ...
        for (occIndex_2=occIndex_First;occIndex_2<occCount_2;occIndex_2++) {
            if (PEIsPairEndMatch(peInput, &(occList_1[occIndex_1]), &(occList_2[occIndex_2]))) {
                ...
                //Output answer
                PEReportPairResult(peInput, peOutput,&(occList_1[occIndex_1]),&(occList_2[occIndex_2]));
        ...
        if (occList_1[occIndex_1].ambPosition < occList_2[occIndex_2].ambPosition) {
            if (occList_1[occIndex_1].strand == strandLeftLeg) {
                for (i=occIndex_2;i<occCount_2;i++) {
                    if (PESRAIsPairEndMatch(peInput, &(occList_1[occIndex_1]), &(occList_2[i]),&insertion)) {
                        ...
                        if (OutputType == PE_REPORT_RANDOM_BEST) {
                            ...
                            PEReportPairResult(peInput, peOutput,peStats,&(occList_1[occIndex_1]),&(occList_2[i]),insertion);
            ...
            if (occList_2[occIndex_2].strand == strandLeftLeg) {
                for (i=occIndex_1;i<occCount_1;i++) {
                    if (PESRAIsPairEndMatch(peInput, &(occList_2[occIndex_2]), &(occList_1[i]),&insertion)) {
                        ...
                        if (OutputType == PE_REPORT_RANDOM_BEST) {
                            ...
                            PEReportPairResult(peInput, peOutput, peStats, &(occList_1[i]), &(occList_2[occIndex_2]),insertion);
                        } else if (OutputType == PE_REPORT_ONE) {
    ...
    if (OutputType == PE_REPORT_RANDOM_BEST && bestCollectorStatus==PE_BEST_AVAILABLE) {
        PEReportPairResult(peInput, peOutput, peStats, &(occList_1[bestPairOccIdx1]), &(occList_2[bestPairOccIdx2]),bestPairInsertion);
    }
    //---------------------------------------------------------
}
...
inline int PEValidateAndPreparePEInput(PEInput * peInput) {
    ...
    if (peInput->OutputType!=PE_REPORT_ALL &&
        peInput->OutputType!=PE_REPORT_ALL_BEST &&
...
void PEMapping(PEInput * peInput, PEOutput * peOutput, PEStats * peStats,
    ...
    //Calling the core
    PEMappingCore(peInput,peOutput,peStats,occList_1,occCount_1,occList_2,occCount_2);
    
    if (peOutput->flag == PE_ALIGNMENT_INITIALISED)
...
int PEIsPairEndMatch(PEInput * peInput, 
    ...
    // strand of the left-read alignment is NOT the same as the strand of the right-read alignment.
        ...
        printf("    pos = %llu \t strand = %d \n",ambPos_left,strand_left);
        printf("    pos = %llu \t strand = %d \n",ambPos_right,strand_right);
    #endif
    
    int strandLeftLeg = peInput->strandLeftLeg;
    int strandRightLeg = peInput->strandRightLeg;
    
    int strandCheck = (strand_left == strandLeftLeg &&
                        strand_right == strandRightLeg);
    
    //positions are left aligned, therefore it requires correction
    unsigned long long ambPos_2 = ambPos_right + matchLen_right - 1;

    unsigned long long gap = ambPos_2 - ambPos_left + 1;
    (*insertion) = gap;
    
    #ifdef PE_DEBUG_PRINT_VISUAL_MATCHING
    char DEBUG_sorted[2];
    DEBUG_sorted[0]='X';DEBUG_sorted[1]=' ';
    printf(" %10llu <----- %10llu -----> %10llu    [%d:%d] %c\n", ambPos_left, gap, ambPos_2, peInput->insertLbound,peInput->insertUbound,DEBUG_sorted[(ambPos_left<=ambPos_2)&1]);
    #endif
...
int PESRAIsPairEndMatch(PEInput * peInput, SRAOccurrence * occ_1, SRAOccurrence * occ_2, unsigned int * insertion) {
    ...
    #ifdef PE_DEBUG_PRINT_MATCHING_PROGRESS
        printf("PESRAIsPairEndMatch is invoked.\n");
        printf("    pos = %llu \t strand = %d \n",occ_1->ambPosition,occ_1->strand);
        printf("    pos = %llu \t strand = %d \n",occ_2->ambPosition,occ_2->strand);
    #endif
    
    return PEIsPairEndMatch(peInput,
                            occ_1->ambPosition,occ_1->strand,occ_1->matchLen,
                            occ_2->ambPosition,occ_2->strand,occ_2->matchLen,
                            insertion);
}

/////////////////////////////////////////////////////
/*
    Function PEDPIsPairEndMatch
    This function takes in 2 DP occurrence and determind
    if the two occurrences form a pair end mapping.
    Assume occ_1 <= occ_2.
    
    Input:
        peInput - The input parameter for PE
        occ_1 - The DP occurrence on left leg
        occ_2 - The DP occurrence on right leg
        insertion - The insertion size output
...
void PEReportPairResult(PEInput * peInput, PEOutput * peOutput, PEStats * peStats, SRAOccurrence * occ_1, SRAOccurrence * occ_2, unsigned int insertion) {
    ...
    if ( peInput->maxResult != -1 && peStats->NumPEAlignments >= peInput->maxResult ) {
        #ifdef PE_DEBUG_PRINT_MATCHING_PROGRESS
            printf("    The total number of alignment found exceeded limitation (%u>%d).\n",peStats->NumPEAlignments,peInput->maxResult);
            printf("    Alignment is discarded\n");
        #endif
    ...
    peStats->NumPEAlignments++;
    peStats->WithError[pePair->occ_1->mismatchCount+pePair->occ_2->mismatchCount]++;
...
PEInput * PEInputMakeClone(PEInput * source) {
    PEInput * destination = (PEInput*) malloc(sizeof(PEInput));
    
    memcpy(destination,source,sizeof(PEInput));
...
PEStats * PESTATConstruct() {
    PEStats * peStats = (PEStats*) malloc ( sizeof(PEStats) );
    PESTATInitialise(peStats);
    return peStats;
}

void PESTATInitialise(PEStats * peStats) {
    memset(peStats,0,sizeof(PEStats));
}
...
unsigned long long PEStatsPEPairList(PEPairList * pairList, PEStats * peStats, PEPairs ** optimal, unsigned int * mismatchStats)
{
    return 0;
}
...
{
    return PEStatsPEPairList ( peOutput->root, peStats, optimal, mismatchStats );
}
