...
BWT *BWTLoad(MMPool *mmPool, const char *bwtCodeFileName, const char *occValueFileName, 
    ...
    unsigned int i;
    FILE *bwtCodeFile, *occValueFile, *cachedSaIndexFile = NULL;
    BWT *bwt;
    unsigned int tmp;
    ...
    if (cachedSaIndexFileName != NULL && cachedSaIndexFileName[0] != '\0' && cachedSaIndexFileName[0] != '-') {
        ...
        if (cachedSaIndexFile == NULL) {
            fprintf(stderr, "BWTLoad() : cannot open cachedSaIndexFile %s!\n",cachedSaIndexFileName);
            exit(1);
        }
    }
    ...
    // MIC Specific BWT Occurrences Auxiliary Data Structure
    unsigned int occSizeMIC = ( (bwt->textLength + MIC_INDEX_OCC_INTERVAL - 1) / MIC_INDEX_OCC_INTERVAL + 1) * ALPHABET_SIZE;
    bwt->micOccSizeInWord = occSizeMIC;
    bwt->micOccValue = (unsigned int *) malloc(occSizeMIC * sizeof(unsigned int));
    unsigned int j;
    for (i=0; i<occSizeMIC/ALPHABET_SIZE ; i++){
        for (j=0; j<ALPHABET_SIZE; j++) {
            bwt->micOccValue[i*ALPHABET_SIZE+j] = BWTOccValueExplicit(bwt,i*2,j);
        }
    }
    ...
    bwt->saValueOnBoundary = NULL;
    BWTSALoad(mmPool,bwt,saValueFileName,inverseSaFileName);
    if (bwt->saValue != NULL) {
...
void BWTSALoad(MMPool *mmPool, BWT * bwt,
             const char *saValueFileName, const char *inverseSaFileName) {

    unsigned int i;
    FILE *saValueFile = NULL, *inverseSaFile = NULL;
    unsigned int tmp;
    
    if (saValueFileName != NULL && saValueFileName[0] != '\0' && saValueFileName[0] != '-') {
        saValueFile = (FILE*)fopen64(saValueFileName, "rb");
        if (saValueFile == NULL) {
            fprintf(stderr, "BWTLoad() : cannot open saValueFile %s!\n",saValueFileName);
            exit(1);
        }
    }
    ...
    if (inverseSaFileName != NULL && inverseSaFileName[0] != '\0' && inverseSaFileName[0] != '-') {
        ...
        if (inverseSaFile == NULL) {
            fprintf(stderr, "BWTLoad() : cannot open inverseSaFile %s!\n",inverseSaFileName);
            exit(1);
        }
    }
...
void BWTFree(MMPool *mmPool, BWT *bwt) {
    ...
    if (bwt->micOccValue != NULL) {
        free(bwt->micOccValue);
    }
    
    BWTSAFree(mmPool, bwt);
...
void BWTSAFree(MMPool *mmPool, BWT *bwt) {
    if (bwt->saValue != NULL) {
...
unsigned int BWTOccValueExplicitOutter(const BWT *bwt, const unsigned int occIndexExplicit, const unsigned int character) {
    return BWTOccValueExplicit(bwt,occIndexExplicit,character);
}
...
/////////////////////////////////////////////////////////////
// Count number of 1's in r1a, to be improved?
// This is only used in BWTDecode* and BWTDecodeAll* currently
/////////////////////////////////////////////////////////////
__attribute__((target(mic)))
inline unsigned int _MICBWTCountBits(__m512i r){

    r = _mm512_add_epi32(_mm512_and_epi32(r,_mm512_set1_epi32(0x33333333)),
        _mm512_and_epi32(_mm512_srli_epi32(r, 2),_mm512_set1_epi32(0x33333333)));
    r = _mm512_and_epi32(_mm512_add_epi32(r,_mm512_srli_epi32(r,4)), _mm512_set1_epi32(0x0F0F0F0F));
    r = _mm512_and_epi32(_mm512_add_epi32(r,_mm512_srli_epi32(r,8)), _mm512_set1_epi32(0x00FF00FF));
    r = _mm512_and_epi32(_mm512_add_epi32(r,_mm512_srli_epi32(r,16)), _mm512_set1_epi32(0x0000FFFF));
   
    return _mm512_reduce_add_epi32(r);    // Extract and return result from register
}

//////////////////////////////////////////////////////////////////////////////////////////////////////
// An MIC specific implementation of BWTDecode.
//////////////////////////////////////////////////////////////////////////////////////////////////////
__attribute__((target(mic)))
unsigned int BWTDecode(const BWT *bwt, const unsigned int index1, const unsigned int index2, const unsigned int character) {

    // prefetch bwt string here  
    _mm_prefetch((const char*)(bwt->bwtCode + ((lastAlignedBoundary(index2, CHAR_PER_512)) / CHAR_PER_WORD)),_MM_HINT_T0);
    
    // mic registers (512-bit)
    register __m512i ri, r1, r2;
    // mic masks (16-bit)
    __mmask16 m1;

    int numChar=index2-index1;
    r1 = _mm512_set1_epi32(-numChar);

    // Set a vector mask such that the counted part are 1's and the rest are 0's
    if (numChar>0){
        m1 = _mm512_cmple_epi32_mask(r1, _mm512_set_epi32(-240,-224,-208,-192,-176,-160,-144,-128,-112,-96,-80,-64,-48,-32,-16,0));
        r2 = _mm512_mask_or_epi32(_mm512_setzero_epi32(),m1,_mm512_setzero_epi32(),_mm512_set1_epi32(0xFFFFFFFF));
        m1 = _mm512_kand(m1,_mm512_cmpge_epi32_mask(r1, _mm512_set_epi32(-255,-239,-223,-207,-191,-175,-159,-143,-127,-111,-95,-79,-63,-47,-31,-15)));
        r1 = _mm512_mask_slli_epi32(r2, m1, r2, (((16 - (numChar%16)))*2)); 
    }
    else{
        m1 = _mm512_cmpge_epi32_mask(r1, _mm512_set_epi32(0,16,32,48,64,80,96,112,128,144,160,176,192,208,224,240));
        r2 = _mm512_mask_or_epi32(_mm512_setzero_epi32(),m1,_mm512_setzero_epi32(),_mm512_set1_epi32(0xFFFFFFFF));
        m1 = _mm512_kand(m1,_mm512_cmple_epi32_mask(r1, _mm512_set_epi32(15,31,47,63,79,95,111,127,143,159,175,191,207,223,239,255)));
        r1 = _mm512_mask_srli_epi32(r2, m1, r2, (((16 - ((-numChar)%16)))*2)); 
    } 

    // load bwt string here
    ri = _mm512_load_epi32(bwt->bwtCode + ((lastAlignedBoundary(index2, CHAR_PER_512)) / CHAR_PER_WORD));
 
    // start counting
    r2 = _mm512_xor_epi32(_mm512_srli_epi32(ri, 1),_mm512_set1_epi32(0xFFFFFFFF + (character >> 1)));
    ri = _mm512_xor_epi32(ri, _mm512_set1_epi32(0xFFFFFFFF + (character & 0x00000001)));

    ri = _mm512_and_epi32(ri, r2);    // Combine even and odd bits
    r1 = _mm512_and_epi32(r1, _mm512_set1_epi32(0x55555555));
    r1 = _mm512_and_epi32(ri, r1);    // Combine encoding with counting mask
    
    return _MICBWTCountBits(r1);    // Extract and return result from register
}

//////////////////////////////////////////////////////////////////////////////////////////////////////
// An MIC specific implementation of BWTDecodeAll. 
//////////////////////////////////////////////////////////////////////////////////////////////////////
// Ordering of index1 and index2 is not important; this module will handle the ordering
__attribute__((target(mic)))
void BWTDecodeAll(const BWT *bwt, const unsigned int index1, const unsigned int index2, unsigned int* __restrict occValue) {
    ...
    r2 = _mm512_set1_epi32(0x55555555); // Set bit-clearing mask to 0x55555555....(alternate 1-bit)

    // Start counting; encoding has been loaded into register earlier
    ri = _mm512_and_epi32(ri, r1);    // Combine encoding with counting mask

    // Shift and combine with character selection mask

    ra1 = _mm512_srli_epi32(ri, 1);    // Shift encoding to right by 1 bit
    rt1 = _mm512_and_epi32(ri, r2);    // Check even-bits = '1'
    rg1 = _mm512_and_epi32(ra1, r2);    // Check odd-bits = '1'
    rc1 = _mm512_andnot_epi32(ri, r2);    // Check even-bits = '0'
    ra1 = _mm512_andnot_epi32(ra1, r2);    // Check odd-bits = '0'

    // Count for 'c' 'g' 't'

    ri = _mm512_and_epi32(ra1, rt1);        // Combine even and odd bits
    ra1 = _mm512_and_epi32(rg1, rc1);        // Combine even and odd bits
    rc1 = _mm512_and_epi32(rg1, rt1);        // Combine even and odd bits
    
    occValue[1] = _MICBWTCountBits(ri);
    occValue[2] = _MICBWTCountBits(ra1);
    occValue[3] = _MICBWTCountBits(rc1);
    occValue[0] -= occValue[1] + occValue[2] + occValue[3];

}

//////////////////////////////////////////////////////////////////////////////////////////////////////
// An MIC specific implementation of BWTAllOccValueTwoIndex is unavailable. 
//////////////////////////////////////////////////////////////////////////////////////////////////////
void BWTAllOccValueTwoIndex(const BWT *bwt, unsigned int index1, unsigned int index2, unsigned int* __restrict occValue1, unsigned int* __restrict occValue2) {
}
...
unsigned int BWTOccValue(const BWT *bwt, unsigned int index, const unsigned int character) {
    ...
    occExplicitIndex = (index + MIC_INDEX_OCC_INTERVAL / 2) / MIC_INDEX_OCC_INTERVAL;
    occIndex = occExplicitIndex * MIC_INDEX_OCC_INTERVAL;

    _mm_prefetch((const char*)(bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE+character)),_MM_HINT_T0);

    if (occIndex != index) {
        decodeValue = BWTDecode(bwt, occIndex, index, character);
        r = -(occIndex > index);
        occValue =  256 + (decodeValue & ~r) - (decodeValue & r);
    } else {
        occValue =  256;
    }

    return occValue + (BWTOccValueExplicitMIC(bwt, occExplicitIndex, character)) - 256;
}
...
static INLINE void BWTAllOccValueExplicit(const BWT *bwt, const unsigned int occIndexExplicit, unsigned int* __restrict occValueExplicit) {
    ...
    unsigned ALIGN_64 t1[16], t2[16];
    memset(t1, 0, sizeof(t1));
    memset(t2, 0, sizeof(t2));
    memcpy(t2, bwt->occValue + (occIndexExplicit / OCC_VALUE_PER_WORD * ALPHABET_SIZE), 16);
    memcpy(t1, bwt->occValueMajor + (occIndexMajor * ALPHABET_SIZE), 16);

    //v2 = _mm512_load_epi32(bwt->occValue + (occIndexExplicit / OCC_VALUE_PER_WORD * ALPHABET_SIZE));
    //v1 = _mm512_load_epi32(bwt->occValueMajor + (occIndexMajor * ALPHABET_SIZE));
    v2 = _mm512_load_epi32(t2);
    v1 = _mm512_load_epi32(t1);


    m = _mm512_set1_epi32(mask);

    v2 = _mm512_srli_epi32(v2, shift);
    v2 = _mm512_and_epi32(v2, m);

    v1 = _mm512_add_epi32(v1, v2);

    _mm512_store_epi32(occValueExplicit, v1);
}
...
void BWTAllOccValue(const BWT *bwt, unsigned int index, unsigned int* __restrict occValue) {
    ...
    _mm_prefetch((const char*)(bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE)),_MM_HINT_T0);
   // _mm_prefetch((const char*)(bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE+1)),_MM_HINT_T0);
   // _mm_prefetch((const char*)(bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE+2)),_MM_HINT_T0);
   // _mm_prefetch((const char*)(bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE+3)),_MM_HINT_T0);

    if (occIndex != index) {
        BWTDecodeAll(bwt, occIndex, index, tempOccValue);
    } else {
        memset(tempOccValue,0,sizeof(tempOccValue)); 
    }

    memcpy(occValue,bwt->micOccValue+(occExplicitIndex*ALPHABET_SIZE),sizeof(unsigned int)*ALPHABET_SIZE);

    int i;   
    if (occIndex > index){
        for (i=0;i<ALPHABET_SIZE;i++)
            occValue[i] -= tempOccValue[i];
    }
    else{
        for (i=0;i<ALPHABET_SIZE;i++)
            occValue[i] += tempOccValue[i];
...
unsigned int BWTOccValueOnSpot(const BWT *bwt, unsigned int index, unsigned int* __restrict character) {
    ...
    *character = bwt->bwtCode[(index - 1) / CHAR_PER_WORD] << (((index - 1) % CHAR_PER_WORD) * BIT_PER_CHAR) >> (BITS_IN_WORD - BIT_PER_CHAR);
    occValue = BWTOccValueExplicitMIC(bwt, occExplicitIndex, *character);
