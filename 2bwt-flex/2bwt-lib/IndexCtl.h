...
void IDXHEnqueueUint32(IndexHeader * idxHeader, uint32_t key32);

// IDXHEnqueueUint32Array enqueue an array of uint32_t value to the back of the header block.
void IDXHEnqueueUint32Array(IndexHeader * idxHeader, uint32_t * key32, uint32_t count);

// IDXHEnqueueRaw enqueue a byte-array to the back of the header block.
void IDXHEnqueueRaw(IndexHeader * idxHeader, uint8_t * key8, uint32_t count);

// IDXHDequeueUint32 dequeue a uint32_t value to the back of the header block.
// and stores it to key32.
void IDXHDequeueUint32(IndexHeader * idxHeader, uint32_t * key32);

// IDXHDequeueUint32Array dequeue an array of uint32_t value to the back of the header block.
// and stores it to key32.
void IDXHDequeueUint32Array(IndexHeader * idxHeader, uint32_t * key32, uint32_t count);

// IDXHDequeueRaw dequeue a byte-array to the back of the header block.
void IDXHDequeueRaw(IndexHeader * idxHeader, uint8_t * key8, uint32_t count);
